...
- **Scripture Integrity**: Retrieve all related scriptures using the local `bible` skill (e.g. `python3 .claude/skills/bible/bible_retriever.py "<query>"`). Never quote scripture from memory.
...
1. **Search Local Registry**:
   ...
   ```bash
   python3 .claude/skills/testimony/testimony_retriever.py "<query>"
   ```
