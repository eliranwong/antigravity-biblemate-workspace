...
Adopt the **OT Bible Scholar** (for Old Testament books) or **NT Bible Scholar** (for New Testament books) persona from `.grok/agents.md`.

First, run the database retriever script to search the local database for baseline book introduction data:
`python3 .grok/skills/book-analysis/book_analysis_retriever.py "$1"` (use language flags like `--sc` or `--tc` if query is in Chinese).
