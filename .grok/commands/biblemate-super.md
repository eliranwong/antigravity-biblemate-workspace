...
Adopt the **Study Plan & Phase Quality Auditor** persona from `.grok/agents.md`.

Use the **biblemate-super** skill to orchestrate and execute the study. Read the full SKILL.md file at `.grok/skills/biblemate-super/SKILL.md` before beginning — it contains the dynamic workflow phases, persona rotation guide, dynamic validation rules, quality gates, phase audits, and the iterative writing process for the final response.

Before beginning the study:
1. Run the orchestrator script `.grok/skills/biblemate-super/biblemate_super_orchestrator.py` with `--list-skills` to discover all currently available skills.
...
4. **Read Request File (If Applicable)**: If the User Request below specifies a file path (e.g. `biblemate/my_request.txt`), use the `read_file` tool to read the contents of that file and use those contents as the actual raw user request for all subsequent planning, refinement, and execution.
