...
# Load promises_dict.py dynamically
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
dict_file = os.path.join(SCRIPT_DIR, 'data', 'promises_dict.py')
PROMISES = {}
...
try:
    ...
    PROMISES = local_vars.get('PROMISES', {})
except Exception as e:
    print(f"Error loading promises database dictionary: {e}", file=sys.stderr)
    sys.exit(1)
...
def get_best_match(query, PROMISES_dict):
    query_clean = query.strip().lower()
    
    # 1. Exact match (case-insensitive) of cleaned key or parts
    for raw_key in PROMISES_dict:
...
def find_db_path():
    ...
    p = os.path.join(base, 'data', 'collections3.sqlite')
    if os.path.exists(p):
        return p
...
def fetch_verse_text(ref, workspace_root):
    retriever_script = os.path.join(workspace_root, '.grok', 'skills', 'bible', 'bible_retriever.py')
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 promises_query.py <promises_topic>", file=sys.stderr)
        sys.exit(1)
        
    query = " ".join(sys.argv[1:])
    best_match = get_best_match(query, PROMISES)
    
    if not best_match:
        print(f"Error: Could not locate a matching Bible promises category for '{query}'.", file=sys.stderr)
        sys.exit(1)
        
    entries = PROMISES[best_match]
    ...
    md = []
    md.append(f"# Bible Promises: {clean_key(best_match)}")
    ...
    try:
        ...
        for tool, number in entries:
            c.execute("SELECT Topic, Passages FROM PROMISES WHERE Tool = ? AND Number = ?", (int(tool), int(number)))
            row = c.fetchone()
            if not row:
                continue
            ...
            for ref in refs:
                ...
                md.append("") # Blank line separator
    ...
    filename = f"{timestamp}_promises_{clean_name}.md"
