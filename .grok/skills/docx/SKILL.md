...
- Always run the python converter script located at `.grok/skills/docx/docx_converter.py` to perform the conversion or export.
- Execute the script using: `python3 .grok/skills/docx/docx_converter.py "<query>"` where `<query>` is the input prompt or arguments containing the file path or export directives (e.g. `whole`, `conversation`).
