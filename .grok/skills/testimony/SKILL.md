...
- **Scripture Integrity**: Retrieve all related scriptures using the local `bible` skill (e.g. `python3 .grok/skills/bible/bible_retriever.py "<query>"`). Never quote scripture from memory.
...
1. **Search Local Registry**:
   ...
   ```bash
   python3 .grok/skills/testimony/testimony_retriever.py "<query>"
   ```
2. **Perform Online Search (if needed)**:
   ...
   - Use the `web_search` tool to search for real-life testimonies, modern missionary reports, or news articles.
   - Use the `web_fetch` / `open_page` tool to fetch pages and verify the details.
