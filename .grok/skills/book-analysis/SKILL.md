...
- Always run the python script located at `.grok/skills/book-analysis/book_analysis_retriever.py` to search the local database for baseline book introductions.
- Execute the script using: `python3 .grok/skills/book-analysis/book_analysis_retriever.py "<Book>"` (you can also pass language options like `--sc` or `--tc` if a Chinese translation is preferred, or the script will auto-detect Chinese characters).
