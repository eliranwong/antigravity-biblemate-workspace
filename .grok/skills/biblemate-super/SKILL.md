...
description: Run a dynamically planned, multi-phase BibleMate-Super study, orchestrating specialized skills and personas with dynamic audit checkpoints. Use when the user runs /biblemate-super or requests this BibleMate workflow.
---
...
The python orchestrator script is located at `.grok/skills/biblemate-super/biblemate_super_orchestrator.py`. Use it for file management:
...
1. **Adopt Assigned Persona**: Read the guidelines in `.grok/agents.md` for that persona and adopt its tone, focus, and vocabulary.
...
> During execution of long or multiple parallel steps, use the `spawn_subagent` tool (with `DurationSeconds="1"`) to dispatch tasks and yield control back to the system. This breaks up execution into smaller agent turns, avoiding the accumulation of huge command outputs in a single conversation history, and allows you to fetch/save step files independently.
...
| Real-life or missionary testimonies, illustration retrieval (`testimony` skill) | **Passionate Evangelist** | Verified real testimonies only (local registry and/or online research with sources); never fabricate |
