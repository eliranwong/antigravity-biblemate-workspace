...
- Always run the python script located at `.grok/skills/chapter-summary/chapter_summary_retriever.py` to search the local database for baseline chapter summaries.
- Execute the script using: `python3 .grok/skills/chapter-summary/chapter_summary_retriever.py "<Book> <Chapter>"` (you can also pass language options like `--sc` or `--tc` if a Chinese translation is preferred, or the script will auto-detect Chinese characters).
